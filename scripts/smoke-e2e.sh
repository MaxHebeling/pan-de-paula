...
echo "▶ smoke web   → $WEB"
E2E_WEB_URL="$WEB" pnpm --filter @pdp/web exec playwright test --grep @smoke --project="$PROJECT" --reporter=list
echo "▶ smoke admin → $ADMIN"
E2E_BASE_URL="$ADMIN" pnpm --filter @pdp/admin exec playwright test --grep @smoke --project="$PROJECT" --reporter=list
echo "✔ Smoke E2E OK"
