  ...
  const [users, roles] = await Promise.all([
    db()
      ...
      .select([
        ...
        "u.last_login_at",
        "u.locked_until",
        sql<number>`(select count(*)::int from staff_sessions s where s.staff_id = u.id and s.revoked_at is null and s.expires_at > now())`.as("sessions"),
      ])
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_380px]">
                ...
                <tr key={u.id}>
                  <td>
                    <div className="font-medium">
                      {u.full_name}
                      {u.id === session.staff.id && <span className="ml-1 text-xs text-muted">(tú)</span>}
                    </div>
                  ...
                  <td className="text-muted">{u.last_login_at ? fmtDate(u.last_login_at, "datetime") : "nunca"}</td>
        ...
        <Card title="Nuevo usuario">
          ...
          >
            ...
            <Select label="Rol" name="role_key" defaultValue={assignable.find((r) => r.key === "cashier")?.key ?? assignable.at(-1)?.key}>
              {assignable.map((r) => (
            ...
            <p className="text-xs text-muted">Se genera una contraseña temporal que verás una sola vez. El usuario deberá cambiarla en su primer acceso.</p>
            <div>
