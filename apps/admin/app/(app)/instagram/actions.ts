...
export async function replyAction(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  const p = z
    .object({
      id: uuid,
      text: z.string().trim().min(1, "Escribe la respuesta").max(1000, "Máximo 1000 caracteres"),
    })
    .safeParse({ id: str(fd, "id"), text: str(fd, "text") });
  if (!p.success) return { error: p.error.issues[0]?.message ?? "Datos inválidos" };
  const { id, text } = p.data;
  const conv = await sql<{
    ig_user_id: string;
    status: string;
  }>`select ig_user_id, status from instagram_conversations where id = ${id}`.execute(db());
  const c = conv.rows[0];
  ...
  try {
    await withStaff(db(), s.staff.id, async (trx) => {
      await sql`insert into instagram_messages(conversation_id, external_mid, direction, text, auto_reply, sent_by, delivery_status, delivery_error)
                values (${id}, ${externalMid}, 'out', ${text}, false, ${s.staff.id}, ${status}, ${deliveryError})`.execute(
        trx,
      );
      await sql`update instagram_conversations set last_message_at = now(), assigned_to = coalesce(assigned_to, ${s.staff.id}),
                status = case when status = 'open' then 'handled' else status end where id = ${id}`.execute(
        trx,
      );
    });
  } catch (e) {
  ...
  if (status === "failed")
    return {
      error: `Meta rechazó el envío (${deliveryError}). El mensaje quedó registrado como fallido.`,
    };
  return {
    ok: "Instagram no está conectado: la respuesta se guardó como PENDIENTE y NO se envió. Cópiala y respóndela desde la app de Instagram.",
  };
}
...
export async function setConversationStatusAction(
  ...
  const s = await requireSession("marketing.write");
  const p = z
    .object({ id: uuid, status: z.enum(["open", "handled", "converted", "closed"]) })
    .safeParse({ id: str(fd, "id"), status: str(fd, "status") });
  if (!p.success) return { error: "Datos inválidos" };
  try {
    await withStaff(db(), s.staff.id, async (trx) => {
      await sql`update instagram_conversations set status = ${p.data.status} where id = ${p.data.id}`.execute(
        trx,
      );
      if (p.data.status === "converted") {
        await sql`update leads set status = 'converted', converted_at = coalesce(converted_at, now()) where source_ref = ${p.data.id} and status <> 'converted'`.execute(
          trx,
        );
      }
    });
...
export async function assignConversationAction(
  ...
  const s = await requireSession("marketing.write");
  const p = z
    .object({ id: uuid, staff_id: uuid.optional() })
    .safeParse({ id: str(fd, "id"), staff_id: optStr(fd, "staff_id") });
  if (!p.success) return { error: "Datos inválidos" };
  try {
    await withStaff(db(), s.staff.id, (trx) =>
      sql`update instagram_conversations set assigned_to = ${p.data.staff_id ?? null} where id = ${p.data.id}`.execute(
        trx,
      ),
    );
    revalidate(p.data.id);
...
export async function linkCustomerAction(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  const p = z
    .object({
      id: uuid,
      query: z.string().trim().min(3, "Escribe código PDP, teléfono o email").max(80),
    })
    .safeParse({ id: str(fd, "id"), query: str(fd, "query") });
  if (!p.success) return { error: p.error.issues[0]?.message ?? "Datos inválidos" };
  try {
    const c = await sql<{
      id: string;
      full_name: string;
    }>`select id, full_name from find_customer(${p.data.query})`.execute(db());
    if (!c.rows[0]) return { error: `No se encontró un cliente con "${p.data.query}"` };
    await withStaff(db(), s.staff.id, async (trx) => {
      await sql`update instagram_conversations set customer_id = ${c.rows[0]!.id} where id = ${p.data.id}`.execute(
        trx,
      );
      await sql`update leads set customer_id = coalesce(customer_id, ${c.rows[0]!.id}) where source_ref = ${p.data.id}`.execute(
        trx,
      );
    });
    revalidate(p.data.id);
...
export async function unlinkCustomerAction(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  try {
    await withStaff(db(), s.staff.id, (trx) =>
      sql`update instagram_conversations set customer_id = null where id = ${p.data}`.execute(trx),
    );
    revalidate(p.data);
  ...
  try {
    await withStaff(db(), s.staff.id, (trx) =>
      sql`insert into leads(source, source_ref, name, handle, phone, interest, product_id, customer_id)
          ...
          from instagram_conversations c left join customers cu on cu.id = c.customer_id where c.id = ${l.id}`.execute(
        trx,
      ),
    );
    revalidate(l.id);
...
export async function updateLeadAction(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  const p = z
    ...
    .safeParse({
      ...
      folio: optStr(fd, "folio"),
      conversation_id: optStr(fd, "conversation_id"),
    });
  if (!p.success) return { error: p.error.issues[0]?.message ?? "Datos inválidos" };
  const l = p.data;
  try {
    ...
    if (l.folio) {
      ...
      }>`select id, customer_id from orders where folio = upper(${l.folio})`.execute(d);
