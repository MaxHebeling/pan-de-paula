...
export default async function CustomerCardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [qr, biz] = await Promise.all([
    ...
    sql<{
      ...
      phone: string | null;
    }>`select name, tagline, instagram_handle, phone from business_settings where id = 1`.execute(
      db(),
    ),
  ]);
  const b = biz.rows[0]!;
  return (
    <>
      <div className="no-print mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted">
            El QR contiene el identificador opaco del cliente: al escanearlo en el POS se acumulan
            puntos. No expone datos personales.
          </p>
        </div>
      ...
      <div className="print-area mx-auto grid max-w-[720px] gap-6 md:grid-cols-2">
        <section
          className="card-lg card flex flex-col items-center gap-3 p-6 text-center"
        ...
        >
          ...
          <img
            src={qr}
            alt={`Código QR del cliente ${c.public_code}`}
            width={220}
            height={220}
          ...
          <div className="text-xs text-muted">
            Presenta esta tarjeta en cada compra para acumular puntos.
          </div>
        </section>
        <section
          className="card-lg card flex flex-col justify-between gap-3 p-6 text-sm"
