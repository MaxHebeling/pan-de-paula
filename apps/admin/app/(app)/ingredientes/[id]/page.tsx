  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={
          <span className="flex flex-wrap items-center gap-2">
            ...
            <Badge tone={ing.is_available ? "green" : "gray"}>
              {ing.is_available ? "Disponible" : "No disponible"}
            </Badge>
            {low && (
      ...
      {sp.creado && (
        <div className="mb-4">
          <Alert tone="green">
            Ingrediente creado. Registra su precio de compra para costear recetas.
          </Alert>
        </div>
      )}
      {sp.error === "en-uso" && (
        <div className="mb-4">
          <Alert tone="amber">
            No se puede eliminar: se usa en recetas. Quítalo de las recetas primero.
          </Alert>
        </div>
      )}
      {unitCost !== null && last && (
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Stat
          ...
          hint={
            last ? (last.package_label ?? formatQty(Number(last.package_qty), base)) : undefined
          }
        />
        <Stat
          label="Stock"
          value={formatQty(stock, base)}
          hint={`mínimo ${formatQty(min, base)}`}
          tone={low ? "red" : undefined}
        />
        <Stat
          label="Recetas"
          value={usages.length}
          hint={usages.length ? "productos que lo usan" : "no se usa aún"}
        />
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_400px]">
        <div className="flex min-w-0 flex-col gap-4">
          ...
          <Card title="Historial de precios">
            {prices.rows.length === 0 ? (
              <p className="text-sm text-muted">
                Sin precios. Sin precio, las recetas que lo usan no pueden costearse.
              </p>
            ) : (
              <Table className="!shadow-none">
                ...
                <tbody>
                  {prices.rows.map((p, i) => {
                    ...
                    const change = prev
                      ? priceChangePct(
                          Math.round(Number(prev.unit_cost) * 1_000_000),
                          Math.round(Number(p.unit_cost) * 1_000_000),
                        )
                      : null;
                    return (
                      <tr key={p.id} className={i === 0 ? "font-medium" : ""}>
                        ...
                        <td>
                          ...
                          <div className="text-xs text-muted">
                            {formatQty(Number(p.package_qty), base)}
                          </div>
                        </td>
                        ...
                        <td
                          className={`text-right tabular-nums ${change === null ? "text-muted" : change > 0 ? "text-red-d" : change < 0 ? "text-green-d" : ""}`}
                        >
          ...
          <Card title="Movimientos de insumo">
            {canWrite && (
              <ActionForm
                action={recordIngredientMovement.bind(null, ing.id)}
                resetOnSuccess
                className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-[160px_1fr_140px_1fr_auto] md:items-end"
              >
                ...
                <TextInput
                  ...
                  step="any"
                  required
                  hint="En corrección usa signo (−) para restar."
                />
                  ...
                  {movements.rows.map((m) => {
                    ...
                    return (
                      <tr key={m.id}>
                        ...
                        <td>
                          <Badge tone={n > 0 ? "green" : m.type === "WASTE" ? "red" : "amber"}>
            ...
            {canWrite ? (
              ...
              >
                ...
                <TextInput
                  ...
                  name="brand"
                  maxLength={60}
                  defaultValue={ing.brand ?? ""}
                />
                <FormGrid>
                  <Select
                    label="Unidad base"
                    name="base_unit"
                    defaultValue={ing.base_unit}
                    hint={prices.rows.length ? "Bloqueada: ya tiene precios." : undefined}
                  >
                ...
                <Checkbox
                  label="Disponible"
                  name="is_available"
                  defaultChecked={ing.is_available}
                />
                <div>
            ...
            <Card title="Zona de riesgo">
              ...
              >
                <span className="text-muted">
                  {usages.length
                    ? "En uso en recetas: no se puede eliminar."
                    : "Quita el ingrediente del catálogo (se conserva el historial)."}
                </span>
