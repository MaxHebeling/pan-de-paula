...
export default async function CouponsPage({
  ...
  const [coupons, stats, products] = await Promise.all([
    listCoupons({ q, status: COUPON_STATUS[status] ? status : undefined }),
    couponStats(),
    activeProducts(),
  ]);
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Stat
          label="Descuento otorgado (30 días)"
          value={<Money cents={stats.discount_30d} compact />}
        />
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[2fr_1fr]">
        <div>
          ...
          {coupons.length === 0 ? (
            <EmptyState
              title="Sin cupones"
              body="Crea el primero para usarlo en mostrador o en la tienda en línea."
              action={
                canWrite ? <LinkButton href="/cupones/nuevo">Nuevo cupón</LinkButton> : undefined
              }
            />
          ) : (
            <Table>
                ...
                {coupons.map((c) => (
                  <tr key={c.id}>
                    <td>
                      <Link
                        href={`/cupones/${c.id}`}
                        className="font-mono font-semibold text-teal-d hover:underline"
                      >
                        {c.code}
                      </Link>
                    ...
                    <td>
                      ...
                      {c.kind === "amount" && c.value_cents !== null && (
                        <Money cents={c.value_cents} compact />
                      )}
                    ...
                    <td className="text-xs">
                      {c.channels.includes("all") ? "todos" : c.channels.join(", ")}
                    </td>
                    <td>
                      <Badge tone={COUPON_STATUS[c.status]!.tone}>
                        {COUPON_STATUS[c.status]!.label}
                      </Badge>
                    </td>
                  </tr>
                ))}
        ...
        <Card title="Probador de cupones">
          <p className="mb-3 text-sm text-muted">
            Simula la validación exactamente como la hace el POS y la tienda (validate_coupon).
          </p>
