...
export default async function SearchPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <>
      <PageHeader title="Buscar" subtitle={r ? `${total} resultado(s) para “${q}”` : "Clientes, pedidos y productos"} />
      <form className="card mb-4 flex flex-wrap items-end gap-2 p-3" action="/buscar" method="get">
        <div className="min-w-[240px] flex-1">
          <label className="label" htmlFor="q">
            Buscar
          </label>
          <input id="q" name="q" className="input" defaultValue={q} placeholder="Nombre, teléfono, código PDP, folio o producto" autoFocus />
        </div>
        <button className="btn btn-primary">Buscar</button>
      </form>
      {r && total === 0 && <EmptyState title="Sin resultados" body="Revisa la ortografía o prueba con el teléfono o el código." />}
      {r && total > 0 && (
                ...
                {r.orders.map((o) => (
                  <li key={o.id} className="py-2">
                    <div className="flex items-center justify-between gap-2">
                      <Link href={`/pedidos?q=${encodeURIComponent(o.folio)}`} className="font-mono font-medium hover:underline">
                        {o.folio}
                      </Link>
              ...
              <ul className="divide-y divide-line text-sm">
                {r.products.map((p) => (
                  <li key={p.id} className="flex items-center justify-between gap-2 py-2">
                    <div>
                      <Link href={`/productos?q=${encodeURIComponent(p.name)}`} className="font-medium hover:underline">
                        {p.name}
                      </Link>
                      <div className="text-xs text-muted">
                        {p.category_name ?? "—"}
