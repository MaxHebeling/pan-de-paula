...
export default async function InventoryReport({
  ...
  const ctx = await reportContext(await searchParams, "inventario", (d) => ({
    from: d.today,
    to: d.today,
  }));
  ...
  const totals = rows.reduce(
    (a, r) => ({
      opening: a.opening + Number(r.opening),
      production: a.production + Number(r.production),
      sales: a.sales + Number(r.sales),
      waste: a.waste + Number(r.waste),
      corrections: a.corrections + Number(r.corrections),
      other: a.other + Number(r.other),
      closing: a.closing + Number(r.closing),
    }),
  ...
  return (
    <ReportShell ctx={ctx} title="Conciliación de inventario">
      <p className="mb-3 text-sm text-muted">
        Final = inicial + producción − ventas − mermas ± correcciones ± otros (devoluciones,
        anulaciones, iniciales). Solo productos con control de stock.
      </p>
      {negatives.length > 0 && (
        <div className="mb-3">
          <Alert tone="amber">
            {negatives.length} producto(s) con existencia negativa al cierre:{" "}
            {negatives.map((n) => n.product_name).join(", ")}. Registra producción o un conteo
            físico.
          </Alert>
        </div>
      )}
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.product_id}>
              ...
              <td className="text-right tabular-nums text-green-d">
                {Number(r.production) ? `+${qty(r.production)}` : "0"}
              </td>
              <td className="text-right tabular-nums">
                {Number(r.sales) ? `−${qty(r.sales)}` : "0"}
              </td>
              <td className="text-right tabular-nums text-red-d">
                {Number(r.waste) ? `−${qty(r.waste)}` : "0"}
              </td>
              <td className="text-right tabular-nums">{qty(r.corrections)}</td>
              <td className="text-right tabular-nums">{qty(r.other)}</td>
              <td
                className={`text-right font-semibold tabular-nums ${Number(r.closing) < 0 ? "text-red-d" : ""}`}
              >
                {qty(r.closing)}
              </td>
            </tr>
