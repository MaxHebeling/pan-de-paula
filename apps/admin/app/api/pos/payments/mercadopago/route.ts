...
import { NotConfiguredError, createPointOrder, createQrOrder, isMercadoPagoConfigured } from "@pdp/integrations";
import { hasPermission } from "@/lib/auth";
import { db, sql, callFn, withStaff } from "@/lib/db";
import { apiSession, dbErrorResponse, getFlags, getOpenRegister, jsonError, readJson } from "@/lib/pos";
...
function mpErrorResponse(e: unknown, kind: "point" | "qr") {
  ...
  if (e instanceof NotConfiguredError) {
    ...
    return jsonError(503, `Mercado Pago no está configurado (${e.envVars.join(", ")}). Cobra con otro método.`, "MP_NOT_CONFIGURED");
  }
  const msg = (e as Error)?.message ?? "";
  console.error(`[pos] error al crear cobro MP ${kind}`, msg);
  if (/pendiente de implementaci/i.test(msg)) {
    return jsonError(503, `El cobro con ${label} de Mercado Pago aún no está disponible en esta versión. Cobra con otro método.`, "MP_NOT_IMPLEMENTED");
  }
  return jsonError(502, `No se pudo iniciar el cobro con ${label}. Intenta de nuevo o cobra con otro método.`, "MP_ERROR");
}
...
export async function POST(req: Request) {
  ...
  if (!parsed.success) return jsonError(400, parsed.error.issues[0]?.message ?? "Datos inválidos", "VALIDATION");
  ...
  if (!isMercadoPagoConfigured())
    return jsonError(503, "Mercado Pago no está configurado (MERCADOPAGO_ACCESS_TOKEN). Cobra con otro método.", "MP_NOT_CONFIGURED");
  const deviceId = process.env.MERCADOPAGO_POINT_DEVICE_ID;
  if (input.kind === "point" && !deviceId)
    return jsonError(503, "Falta configurar la terminal (MERCADOPAGO_POINT_DEVICE_ID).", "MP_NOT_CONFIGURED");
  ...
  try {
    orderId = await withStaff(db(), staffId, (trx) =>
      callFn<string>(trx, "create_order", [
        JSON.stringify({ ...orderInput, channel: "pos", register_session_id: register?.id ?? null }),
      ]),
    );
    const o = await sql<{
      folio: string;
    ...
    if (row.status === "cancelled") {
      return jsonError(409, "Este intento de cobro ya fue cancelado. Inicia uno nuevo.", "ORDER_CANCELLED");
    }
    ...
    if (prev?.external_id) {
      const out: MpStartResult = { orderId, folio, totalCents, mpOrderId: prev.external_id, qrData: prev.metadata?.qr_data };
      return NextResponse.json(out);
    }
    if (totalCents <= 0) {
      return jsonError(409, "El total es $0: cobra sin Mercado Pago (la recompensa cubre todo).", "ZERO_TOTAL");
    }
  } catch (e) {
  ...
  try {
    ...
    if (kind === "point") {
      const r = await createPointOrder({ deviceId: deviceId!, amountCents: totalCents, externalReference: orderId, description });
      mpOrderId = r.orderId;
    } else {
      const items = await sql<{ product_name: string; qty: string | number; unit_price_cents: number }>`
        select product_name, qty, unit_price_cents from order_items where order_id = ${orderId}::uuid order by sort_order`.execute(db());
      const r = await createQrOrder({
        amountCents: totalCents,
        externalReference: orderId,
        description,
        items: items.rows.map((i) => ({ title: i.product_name, quantity: Number(i.qty), unitPriceCents: i.unit_price_cents })),
      });
      mpOrderId = r.orderId;
      qrData = r.qrData;
    }
    ...
    try {
      ...
      console.error("[pos] no se pudo cancelar pedido tras falla MP", orderId, (cancelErr as Error).message);
    }
