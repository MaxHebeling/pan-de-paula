...
export function parseRange(sp: Record<string, string | string[] | undefined>, defaults: { from: string; to: string }): Range {
...
export type Summary = {
  ...
  orders: { count: number; web: number; pos: number; other: number; cancelled: number; open: number };
...
export const summary = (from: string, to: string) => callFn<Summary>(db(), "report_summary", [from, to]);
...
export const dailySeries = async (from: string, to: string) =>
  (await sql<DailyRow>`select day::text as day, sales_count, revenue_cents, refunds_cents, units, waste_qty, production_qty, new_customers, cost_cents from report_daily_series(${from}, ${to})`.execute(db())).rows;
...
export const products = async (from: string, to: string) => (await sql<ProductRow>`select * from report_products(${from}, ${to})`.execute(db())).rows;
...
export type CustomersReport = {
  ...
  recurring: number;
  returning: number;
  top: Array<{ id: string; public_code: string; full_name: string; tier_key: string | null; sales: number; spent_cents: number; last_purchase_at: string }>;
  tiers: Array<{ tier_key: string; name: string; rank: number; customers: number; sales: number; spent_cents: number }>;
  inactive_30: number;
  inactive_60: number;
...
export const customers = (from: string, to: string) => callFn<CustomersReport>(db(), "report_customers", [from, to]);

export type WasteRow = { reason: string; count: number; qty: string; cost_cents: number };
export const waste = async (from: string, to: string) => (await sql<WasteRow>`select * from report_waste(${from}, ${to})`.execute(db())).rows;

export const wasteDetail = async (from: string, to: string) =>
  (
    await sql<{ id: string; occurred_at: Date; product_name: string; qty: string; reason: string; note: string | null; staff_name: string | null; cost_cents: number }>`
      ...
      where w.occurred_at >= rr.v_from and w.occurred_at < rr.v_to order by w.occurred_at desc limit 500`.execute(db())
  ).rows;

export type ReconRow = { product_id: string; product_name: string; opening: string; production: string; sales: string; waste: string; corrections: string; other: string; closing: string };
export const reconciliation = async (from: string, to: string) =>
  (await sql<ReconRow>`with rr as (select * from report_range(${from}, ${to})) select r.* from rr, inventory_reconciliation(rr.v_from, rr.v_to) r`.execute(db())).rows;
...
export const registerSessions = async (from: string, to: string) =>
  (
    await sql<RegisterRow>`
      ...
      where rs.opened_at >= rr.v_from and rs.opened_at < rr.v_to order by rs.opened_at desc`.execute(db())
  ).rows;
...
export const inactiveCustomers = async (days: number, limit = 50) =>
  (
    await sql<{ id: string; public_code: string; full_name: string; phone: string | null; last_purchase_at: Date; total_orders: number; total_spent_cents: number; tier_key: string | null }>`
...
// ── Utilidades ──
export const PAYMENT_LABELS: Record<string, string> = { cash: "Efectivo", mercadopago: "Mercado Pago", card_terminal: "Terminal", transfer: "Transferencia", points: "Puntos", other: "Otro" };
...
export const pesos = (cents: number | string | null | undefined) => (cents === null || cents === undefined ? "" : (Number(cents) / 100).toFixed(2));
...
export type ReportKind = "diario" | "mensual" | "productos" | "clientes" | "canales" | "mermas" | "inventario" | "caja";
...
export async function exportRows(kind: ReportKind, from: string, to: string): Promise<{ filename: string; columns: Array<{ key: string; label: string }>; rows: Array<Record<string, unknown>> }> {
  ...
  switch (kind) {
    ...
    case "mensual": {
      ...
      return {
        ...
        rows: rows.map((r) => ({ ...r, revenue: pesos(r.revenue_cents), refunds: pesos(r.refunds_cents), net: pesos(r.revenue_cents - r.refunds_cents), cost: pesos(r.cost_cents) })),
      };
    }
    case "productos": {
      ...
      return {
        ...
        rows: rows.map((r) => ({ ...r, revenue: pesos(r.revenue_cents), cost: pesos(r.cost_cents), profit: pesos(r.profit_cents), margin: r.margin_bps === null ? "" : (r.margin_bps / 100).toFixed(1) })),
      };
    }
    case "clientes": {
    ...
    case "canales": {
      ...
      return {
        ...
        rows: [
          ...s.channels.map((c) => ({ type: "Canal", name: c.channel, count: c.count, amount: pesos(c.revenue_cents), refunded: "" })),
          ...s.payments.map((p) => ({ type: "Método de pago", name: PAYMENT_LABELS[p.method] ?? p.method, count: p.count, amount: pesos(p.amount_cents), refunded: pesos(p.refunded_cents) })),
        ],
      };
    }
    case "mermas": {
      ...
      return {
        ...
        rows: rows.map((r) => ({ ...r, reason_label: WASTE_LABELS[r.reason] ?? r.reason, cost: pesos(r.cost_cents) })),
      };
    }
    case "inventario": {
