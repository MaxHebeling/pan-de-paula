        ...
        <div className="grid grid-cols-3 gap-2">
          {[0, 50000, 100000].map((v) => (
            <button
              ...
              type="button"
              onClick={() => setCents(v)}
              className={`btn min-h-11 tabular-nums ${cents === v ? "btn-primary" : "btn-secondary"}`}
            >
    ...
    <form action={action} className="grid gap-4 md:grid-cols-2" data-testid="register-close-form">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className={`card p-3 ${tone}`}>
          ...
          {touched && diff !== 0 && (
            <div className="text-xs">
              {diff > 0 ? "Sobra efectivo" : "Falta efectivo"}: revisa antes de cerrar.
            </div>
          )}
        </div>
        <button
        ...
        <label className="flex min-h-11 items-center gap-2 text-sm">
          <input type="checkbox" name="confirm" className="h-5 w-5" required /> Confirmo el conteo;
        ...
        <button
          className="btn btn-danger btn-lg min-h-14"
          disabled={pending || !touched}
