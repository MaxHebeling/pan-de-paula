...
export function RegisterSummaryTable({
  s,
  compact = false,
}: {
  s: RegisterSummary;
  ...
  const rows: Array<{ label: string; value: string; strong?: boolean; tone?: string }> = [
    ...
    ...(s.refunds_cash_cents
      ? [{ label: "Reembolsos en efectivo", value: `−${formatMXN(s.refunds_cash_cents)}` }]
      : []),
    ...
    ...(s.refunds_other_cents
      ? [{ label: "Reembolsos (no efectivo)", value: `−${formatMXN(s.refunds_other_cents)}` }]
      : []),
    {
      label: `Ventas (${s.sales_count})${s.voided_count ? ` · ${s.voided_count} anuladas` : ""}`,
      value: formatMXN(s.sales_total_cents),
      strong: true,
    },
  ];
  return (
      ...
      <tbody>
        {rows.map((r) => (
          <tr key={r.label} className="border-t border-line first:border-0">
            ...
            <td
              className={`py-1.5 text-right tabular-nums ${r.strong ? "font-semibold" : ""} ${r.tone ?? ""}`}
              data-testid={r.label === "Diferencia" ? "summary-difference" : undefined}
            >
              {r.value}
            </td>
          </tr>
