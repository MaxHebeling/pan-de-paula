 ...
 */
export function QrScanner({ onDetected, onClose }: { onDetected: (value: string) => void; onClose: () => void }) {
  ...
  const error = supported ? cameraError : "Este navegador no puede leer QR con la cámara. Usa el lector USB o escribe el código.";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setCameraError("No se pudo abrir la cámara. Revisa el permiso del navegador o usa el lector USB.");
      }
    })();
    return () => {
      done = true;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" role="dialog" aria-modal="true" aria-label="Escanear QR">
      <div className="card w-full max-w-md overflow-hidden p-0">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <button type="button" className="btn btn-secondary btn-sm min-h-11 min-w-11" onClick={onClose} aria-label="Cerrar">
            <X size={18} />
          </button>
        </div>
        {error ? (
