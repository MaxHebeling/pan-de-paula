...
export function CompareStat({
  ...
  const text =
    d === null
      ? "sin base anterior"
      : `${d > 0 ? "+" : ""}${d.toLocaleString("es-MX")}% vs. periodo anterior${format ? ` (${format(previous)})` : ""}`;
  return (
    <Stat
      label={label}
      value={value}
      hint={hint ? `${hint} · ${text}` : text}
      tone={good === undefined ? undefined : good ? "green" : "red"}
    />
  );
}
...
export function Delta({
  ...
  return (
    <span
      className={`text-xs tabular-nums ${good === undefined ? "text-muted" : good ? "text-green-d" : "text-red-d"}`}
    >
      {d > 0 ? "+" : ""}
  ...
  return (
    <div className="mb-3">
      ...
      <p className="text-sm text-muted">
        {from === to ? f(from) : `${f(from)} – ${f(to)}`} · comparado con{" "}
        {prevFrom === prevTo ? f(prevFrom) : `${f(prevFrom)} – ${f(prevTo)}`}
      </p>
    </div>
  );
}
