...
export function RecipeEditor({
  ...
  const preview = useMemo(() => {
    ...
    const items = domainLines
      .filter((l) => l.ingredientId && l.qty > 0)
      .map((l) => ({
        ingredient_id: l.ingredientId,
        qty: l.qty,
        note: lines.find((x) => x.key === l.key)?.note || null,
      }));
                    ...
                    <tr key={l.key} className="border-t border-line">
                      <td>
                        ...
                        {ing && ing.unit_cost === null && (
                          <span className="text-xs text-amber-d">sin precio: no suma al costo</span>
                        )}
                      ...
                      <td className="text-right tabular-nums">
                        {pl?.cost === null || pl?.cost === undefined
                          ? "—"
                          : `$${pl.cost.toFixed(2)}`}
                      </td>
                      <td>
      ...
      <section className="grid gap-3 md:grid-cols-2">
          ...
          {preview ? (
            <dl className="mt-1 grid grid-cols-2 gap-x-3 gap-y-1 tabular-nums">
              ...
              <dd className="text-right text-base font-semibold">
                {formatMXN(preview.costPerPieceCents)}
              </dd>
              {preview.hasMissingPrices && (
                <dd className="col-span-2 text-xs text-amber-d">
        ...
        <div className="card px-4 py-3 text-sm">
          ...
          <dl className="mt-1 grid grid-cols-2 gap-x-3 gap-y-1 tabular-nums">
            ...
            <dd
              className={`text-right font-semibold ${preview?.margin !== null && preview?.margin !== undefined && preview.margin < 3000 ? "text-red-d" : ""}`}
            >
              {preview?.margin === null || preview?.margin === undefined
                ? "—"
