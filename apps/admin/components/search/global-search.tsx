...
function toItems(r: Results): Item[] {
  return [
    ...r.customers.map((c) => ({
      ...
      href: `/clientes/${c.id}`,
      title: c.full_name,
      sub: `${c.public_code}${c.phone ? ` · ${c.phone}` : ""} · ${c.points_balance} pts`,
      group: "Clientes",
    })),
    ...r.orders.map((o) => ({
      key: `o-${o.id}`,
    ...
    ...r.products.map((p) => ({
      ...
      title: p.name,
      sub: `${p.category_name ?? ""}${p.price_cents !== null ? ` · ${mxn(p.price_cents)}` : ""}${p.on_hand !== null ? ` · stock ${Number(p.on_hand)}` : ""}`,
      group: "Productos",
    })),
  ];
}
...
export function GlobalSearch() {
  ...
  const rows = visible.map((it, i) => ({
    ...it,
    showGroup: i === 0 || visible[i - 1]!.group !== it.group,
  }));
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="fixed inset-0 z-50 flex items-start justify-center bg-black/30 p-3 pt-[10vh]"
          onClick={close}
        >
          <div
            role="dialog"
            ...
            className="card card-lg w-full max-w-xl overflow-hidden shadow-lift"
            ...
            <div className="max-h-[60vh] overflow-y-auto">
              ...
              {!error && q.trim().length < 2 && (
                <p className="px-4 py-3 text-sm text-muted">
                  Escribe al menos 2 caracteres. Enter abre la página de resultados.
                </p>
              )}
              {!error && q.trim().length >= 2 && !loading && visible.length === 0 && (
                <p className="px-4 py-3 text-sm text-muted">Sin resultados para “{q.trim()}”.</p>
              )}
              ...
              {q.trim().length >= 2 && (
                <button
                  type="button"
                  className="block w-full border-t border-line px-4 py-2 text-left text-sm text-teal-d hover:bg-black/[0.03]"
