...
export function VariantPicker({ base, variants }: { base: CatalogProduct; variants: CatalogProduct[] }) {
        ...
        <fieldset>
          ...
          <div className="flex flex-wrap gap-2">
            {variants.map((v) => {
              ...
              return (
                <label key={v.id} className={`chip cursor-pointer ${v.id === selectedId ? "chip-active" : ""} ${!vav.canAdd ? "opacity-50" : ""}`}>
                  <input
                    type="radio"
                    name="variant"
                    value={v.id}
                  ...
                  {v.priceCents !== null && <span className="ml-2 opacity-80">{(v.priceCents / 100).toLocaleString("es-MX", { style: "currency", currency: "MXN", maximumFractionDigits: 0 })}</span>}
                </label>
              );
            })}
          </div>
      ...
      {av.canAdd ? (
        <AddToCart
          product={{
            ...
            slug: base.slug,
            name: variants.length > 0 ? `${base.name} · ${selected.variantLabel ?? selected.name}` : base.name,
        ...
        <p className="rounded-[14px] bg-cream-2 px-4 py-3 text-sm text-ink">Por ahora no está disponible para pedidos en línea.</p>
      )}
    </div>
  );
}
