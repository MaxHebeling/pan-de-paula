...
describe("seguridad: RLS, grants y funciones", () => {
  ...
  it("anon/authenticated/PUBLIC no tienen privilegios de tabla, secuencia, esquema ni EXECUTE en NINGUNA función (0010–0070 y pg_trgm incluidas)", async () => {
    const tbl = await sql<{ n: number }>`
      select count(*)::int as n from information_schema.role_table_grants
      where table_schema = 'public' and grantee in ('anon','authenticated','PUBLIC')`.execute(db);
    expect(tbl.rows[0]!.n).toBe(0);
    const schema = await one(sql<{ anon: boolean; auth: boolean; pub: boolean }>`
      select has_schema_privilege('anon','public','usage') as anon,
             has_schema_privilege('authenticated','public','usage') as auth,
             has_schema_privilege('public','public','usage') as pub`);
    expect(schema).toEqual({ anon: false, auth: false, pub: false });
  ...
  it("una función nueva NO nace ejecutable por PUBLIC (privilegio por defecto global corregido en 0015)", async () => {
    await sql`create function public.__audit_probe() returns int language sql as 'select 1'`.execute(
      db,
    );
    try {
      const r = await one(sql<{ pub: boolean; app: boolean }>`
      ...
      expect(r).toEqual({ pub: false, app: true });
    } finally {
  ...
  it("append-only: inventory_movements, loyalty_transactions, ingredient_movements y domain_events rechazan update/delete", async () => {
    ...
    for (const t of ["inventory_movements", "loyalty_transactions", "domain_events"]) {
      await expect(
        sql`update ${sql.raw(t)} set created_at = now()`.execute(db),
      ).rejects.toThrow(/solo inserción|append-only|does not exist/);
...
describe("apply_mercadopago_payment: conciliación de montos y estados", () => {
  it("monto exacto: pago, venta, inventario y puntos una sola vez; segundo aviso duplicado", async () => {
    ...
    const pts = await one(
      sql<{ s: number }>`select coalesce(sum(points),0)::int as s from loyalty_transactions where customer_id = ${cust.customer_id} and kind = 'earn'`,
    );
    ...
    expect(await count("notifications", "kind in ('payment_mismatch','payment_on_cancelled_order')")).toBe(0);
  });
  ...
  it("monto MENOR al saldo: se detecta → pago parcial, sin venta, alerta payment_mismatch", async () => {
    ...
    expect(await orderState(orderId)).toMatchObject({ payment_status: "partial", paid_cents: 5000 });
    expect(await count("sales")).toBe(0);
    const n = await one(
      sql<{ body: string; severity: string }>`select body, severity from notifications where kind = 'payment_mismatch'`,
    );
    expect(n.severity).toBe("error");
    expect(n.body).toMatch(/saldo \$120\.00.*acreditó \$50\.00.*9002/);
    const pay = await one(
      sql<{ amount_cents: number; metadata: Record<string, unknown> }>`select amount_cents, metadata from payments`,
    );
    expect(pay.amount_cents).toBe(5000);
    expect(pay.metadata).toMatchObject({ mp_amount_cents: 5000, expected_cents: 12000, amount_mismatch: true });
  ...
  it("monto MAYOR al saldo: se registra el saldo del pedido, se concreta la venta y la alerta pide reembolsar la diferencia", async () => {
    ...
    const n = await one(sql<{ body: string }>`select body from notifications where kind = 'payment_mismatch'`);
  ...
  it("pedido ya pagado + segundo pago distinto (cliente pagó dos veces): no se registra, alerta de duplicado", async () => {
    ...
    expect(r).toMatchObject({ applied_status: "needs_refund", recorded: false, reason: "pedido ya pagado" });
    expect(await count("payments")).toBe(1);
    expect(await count("sales")).toBe(1);
    const n = await one(sql<{ title: string; body: string }>`select title, body from notifications where kind = 'payment_mismatch'`);
  ...
  it("pedido cancelado: approved → needs_refund con alerta (sin pago ni venta); pending/rejected → ignored", async () => {
    ...
    expect(r).toMatchObject({ applied_status: "needs_refund", recorded: false, reason: "pedido cancelado" });
    ...
    expect(await apply(mp(orderId, "9008", "rejected", 12000))).toMatchObject({ applied_status: "ignored" });
    expect(await count("payments")).toBe(0);
  });
  ...
  it("fuera de orden: approved y luego pending/rejected del mismo pago no revierten la venta", async () => {
    ...
    expect(await apply(mp(orderId, "9010", "pending", 12000))).toMatchObject({ ignored_transition: "pending" });
    expect(await apply(mp(orderId, "9010", "rejected", 12000))).toMatchObject({ ignored_transition: "failed" });
    expect(await count("sales")).toBe(1);
...
describe("job_runs: lock único y liberación de locks huérfanos (trigger 0015)", () => {
  it("dos running con el mismo lock_key → 23505; uno huérfano (> 15 min) se libera al insertar el siguiente", async () => {
    await sql`insert into job_runs(job_name, status, lock_key) values ('j', 'running', 'j')`.execute(db);
    await expect(
      sql`insert into job_runs(job_name, status, lock_key) values ('j', 'running', 'j')`.execute(db),
    ).rejects.toMatchObject({ code: "23505" });
    await sql`update job_runs set started_at = now() - interval '20 minutes' where lock_key = 'j'`.execute(db);
    const r = await one(
      sql<{ id: string }>`insert into job_runs(job_name, status, lock_key) values ('j', 'running', 'j') returning id`,
    );
  ...
  it("locks distintos no se estorban; un run sin lock_key nunca bloquea", async () => {
    await sql`insert into job_runs(job_name, status, lock_key) values ('a', 'running', 'a'), ('b', 'running', 'b')`.execute(db);
    await sql`insert into job_runs(job_name, status) values ('a', 'running'), ('a', 'running')`.execute(db);
...
describe("integridad: invariantes tras venta, anulación y reembolso", () => {
  it("inventario = suma de movimientos, puntos = ledger, paid_cents = pagos, cupones = redenciones; rebuild repara una deriva inducida", async () => {
    const cust = await createCustomer(db);
    await sql`insert into coupons(code, kind, value_bps, is_active) values ('AUD10', 'pct', 1000, true)`.execute(db);
    const r1 = await withStaff(db, staff, (trx) =>
      callFn<{ order_id: string; sale_id: string }>(trx, "pos_checkout", [
        JSON.stringify({
          ...
          customer_id: cust.customer_id,
          coupon_code: "AUD10",
          payments: [{ provider: "cash", method: "cash", amount_cents: 5400, tendered_cents: 10000 }],
        }),
      ]),
    );
    ...
    const pay = await one(sql<{ id: string }>`select id from payments where order_id = ${r1.order_id}`);
    await withStaff(db, staff, (trx) =>
      callFn(trx, "record_refund", [
        JSON.stringify({ payment_id: pay.id, amount_cents: 1000, reason: "parcial", idempotency_key: "aud-ref-1" }),
      ]),
    );

    const drift = await sql`
      select 1 from inventory_levels l join (select product_id, sum(qty) t from inventory_movements group by 1) m using (product_id) where l.on_hand <> m.t`.execute(db);
    expect(drift.rows).toEqual([]);
    const pts = await sql`
      select 1 from customers c left join (select customer_id, sum(points) s from loyalty_transactions group by 1) t on t.customer_id = c.id where c.points_balance <> coalesce(t.s, 0)`.execute(db);
    ...
    const paid = await sql`
      select 1 from orders o left join (select order_id, sum(amount_cents) s from payments where status in ('paid','partially_refunded','refunded') group by 1) p on p.order_id = o.id where o.paid_cents <> coalesce(p.s, 0)`.execute(db);
    expect(paid.rows).toEqual([]);
    const refunded = await sql`
      select 1 from orders o left join (select order_id, sum(amount_cents) s from refunds where status <> 'failed' group by 1) r on r.order_id = o.id where o.refunded_cents <> coalesce(r.s, 0)`.execute(db);
    expect(refunded.rows).toEqual([]);
    const coup = await sql`
      select 1 from coupons c left join (select coupon_id, count(*) n from coupon_redemptions group by 1) r on r.coupon_id = c.id where c.uses_count <> coalesce(r.n, 0)`.execute(db);
    ...
    const stats = await sql`
      ...
      where c.total_orders <> coalesce(s.n, 0) or c.total_spent_cents <> coalesce(s.t, 0) - coalesce(rf.t, 0)`.execute(db);
...
describe("migraciones y scripts operativos", () => {
  ...
  it("reset.ts rechaza hosts remotos y APP_ENV=production sin conectarse", () => {
    const remote = spawnSync("pnpm", ["exec", "tsx", "scripts/reset.ts"], {
      cwd: PKG,
      env: { ...process.env, DATABASE_URL: "postgres://user:pw@db.example.supabase.co:5432/postgres", APP_ENV: "development" },
      encoding: "utf8",
    });
