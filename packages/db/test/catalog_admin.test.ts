...
async function costOf(productId: string) {
  const r = await sql<{ c: number | null }>`select product_cost_cents(${productId}) as c`.execute(
    db,
  );
  return r.rows[0]!.c;
}
...
describe("upsert_recipe + product_cost_cents", () => {
  it("crea la receta, calcula el costo por pieza y reemplaza líneas al re-guardar", async () => {
    ...
    const again = await withStaff(db, staff, (trx) =>
      callFn<string>(trx, "upsert_recipe", [
        croissant,
        10,
        null,
        0,
        0,
        null,
        JSON.stringify([{ ingredient_id: harina, qty: 500 }]),
      ]),
    );
    ...
    const lines = await sql<{
      n: number;
      version: number;
    }>`select (select count(*)::int from recipe_items where recipe_id = ${recipeId}) as n, version from recipes where id = ${recipeId}`.execute(
      db,
    );
  ...
  it("rechaza rendimiento cero y cantidades no positivas", async () => {
    await expect(
      withStaff(db, staff, (trx) =>
        callFn(trx, "upsert_recipe", [croissant, 0, null, 0, 0, null, "[]"]),
      ),
    ...
    await expect(
      withStaff(db, staff, (trx) =>
        callFn(trx, "upsert_recipe", [
          ...
          0,
          null,
          JSON.stringify([{ ingredient_id: harina, qty: 0 }]),
        ]),
      ),
...
describe("record_ingredient_price + product_cost_impact", () => {
  ...
  it("previsualiza el impacto del nuevo costo unitario en los productos afectados", async () => {
    ...
    const r = await sql<{ product_id: string; current_cost_cents: number; new_cost_cents: number }>`
      select product_id, current_cost_cents, new_cost_cents from product_cost_impact(${mantequilla}, 0.30)`.execute(
      db,
    );
  ...
  it("registra el precio en el historial, recalcula el costo y opcionalmente suma stock", async () => {
    const id = await withStaff(db, staff, (trx) =>
      callFn<string>(trx, "record_ingredient_price", [
        mantequilla,
        1000,
        30000,
        "Barra 1 kg",
    ...
    const price = await sql<{
      ...
      source: string;
      created_by: string;
    }>`select unit_cost, source, created_by from ingredient_prices where id = ${id}`.execute(db);
    ...
    const stock = await sql<{
      stock_qty: string;
    }>`select stock_qty from ingredients where id = ${mantequilla}`.execute(db);
    expect(Number(stock.rows[0]!.stock_qty)).toBe(2000);
    const mv = await sql<{
      type: string;
      qty: string;
    }>`select type, qty from ingredient_movements where ingredient_id = ${mantequilla}`.execute(db);
  ...
  it("valida contenido y precio", async () => {
    await expect(
      withStaff(db, staff, (trx) =>
        callFn(trx, "record_ingredient_price", [mantequilla, 0, 100, null, null, false, 1]),
      ),
    ).rejects.toThrow(/contenido/);
    await expect(
      withStaff(db, staff, (trx) =>
        callFn(trx, "record_ingredient_price", [mantequilla, 10, -5, null, null, false, 1]),
      ),
...
describe("precios: set_regular_price / create_promotion / end_promotion", () => {
  it("cierra el regular anterior y activa el nuevo sin tocar el histórico", async () => {
    const first = await sql<{
      id: string;
    }>`select id from product_prices where product_id = ${croissant}`.execute(db);
    const newId = await withStaff(db, staff, (trx) =>
      callFn<string>(trx, "set_regular_price", [croissant, "all", 4800, "Ajuste 2026"]),
    );
    ...
    const rows = await sql<{
      ...
      select id, price_cents, valid_to, created_by from product_prices where product_id = ${croissant} order by valid_from`.execute(
      db,
    );
  ...
  it("precio por canal: pos distinto de web", async () => {
    await withStaff(db, staff, (trx) =>
      callFn(trx, "set_regular_price", [croissant, "web", 5000, null]),
    );
  ...
  it("promoción vigente gana al regular y se puede terminar anticipadamente", async () => {
    const promoId = await withStaff(db, staff, (trx) =>
      callFn<string>(trx, "create_promotion", [
        croissant,
        "all",
        3900,
    ...
    const row = await sql<{
      valid_to: Date | null;
    }>`select valid_to from product_prices where id = ${promoId}`.execute(db);
    expect(row.rows[0]!.valid_to).not.toBeNull();
    await expect(
      withStaff(db, staff, (trx) => callFn(trx, "end_promotion", [promoId])),
  ...
  it("una promoción futura que se cancela se elimina y una promo >= regular se rechaza", async () => {
    ...
    const gone = await sql<{
      n: number;
    }>`select count(*)::int as n from product_prices where id = ${promoId}`.execute(db);
...
describe("product_has_sales", () => {
  it("es falso sin ventas y verdadero cuando el producto (o una variante) fue vendido", async () => {
    ...
    const variant = await sql<{
      id: string;
    }>`insert into products(name, slug, parent_id, variant_label) values ('Croissant mini', 'croissant-mini', ${croissant}, 'Mini') returning id`.execute(
      db,
    );
    await withStaff(db, staff, (trx) =>
      callFn(trx, "record_production", [variant.rows[0]!.id, 5, null, null]),
    );
    await sql`insert into product_prices(product_id, price_cents) values (${variant.rows[0]!.id}, 2000)`.execute(
      db,
    );
    await withStaff(db, staff, (trx) =>
